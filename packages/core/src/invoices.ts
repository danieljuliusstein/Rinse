...
import { getDocumentStrings, normalizeDocumentLocale, type DocumentLocale } from './document-i18n'

export const OVERDUE_DAYS = 3

export const PAYMENT_METHODS = ['Cash', 'Card', 'Venmo', 'Zelle', 'CashApp', 'Check'] as const
...
export function deriveInvoiceStatus(
  ...
  const effectiveSent =
    sent_at || (current === 'sent' || current === 'partial' || current === 'overdue' ? new Date().toISOString() : undefined)
