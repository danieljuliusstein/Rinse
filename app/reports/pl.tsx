...
import type { BusinessExpense, Invoice, JobWithRelations, OverheadExpense } from '@rinse/core'
...
export default function RevenuePayoutsScreen() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      const [rows, inv, biz, overhead] = await Promise.all([
        listJobs(500),
        listInvoices(),
  ...
  const bundle = useMemo(() => {
    const withCosts = (start: Date, end: Date) => {
      ...
      return computePLReportForDates(jobs, start, end, overhead, business)
    }

    if (range === 'custom' && customStart && customEnd) {
      const start = new Date(customStart + 'T00:00:00')
      const end = new Date(customEnd + 'T23:59:59')
      const current = withCosts(start, end)
      const span = end.getTime() - start.getTime()
      const priorEnd = new Date(start.getTime() - 1)
      const priorStart = new Date(priorEnd.getTime() - span)
      return { current, prior: withCosts(priorStart, priorEnd) }
    }

    const key: DateRangeKey = range === 'custom' ? 'this_month' : range
    const { start, end } = rangeFor(key)
    const prior = priorRangeFor(key)
    return {
      current: withCosts(start, end),
      prior: withCosts(prior.start, prior.end),
    }
  }, [jobs, range, customStart, customEnd, businessExpenses, overheadItems])
  ...
  return (
    <SettingsScreen
      title={t('business.revenuePayouts', { defaultValue: 'Revenue & Payouts' })}
      fallbackHref="/(tabs)/reports"
    >
      <View style={{ marginBottom: spacing.sm }}>
        <AppText variant="caption" style={{ color: colors.textMuted }}>
          {`${rangeLabel} · ${report.jobCount} ${report.jobCount === 1 ? 'job' : 'jobs'}`}
        </AppText>
      </View>
