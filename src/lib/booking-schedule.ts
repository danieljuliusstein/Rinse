...
export function normalizeBookingSchedule(raw: unknown): BookingSchedule {
  ...
  return {
    ...
    end_time:
      typeof o.end_time === 'string' && o.end_time ? o.end_time : DEFAULT_BOOKING_SCHEDULE.end_time,
...
/** Inter-job gap used by availability (buffer + drive pad). */
export function effectiveJobBufferMinutes(schedule: BookingSchedule): number {
  return Math.max(0, schedule.buffer_minutes) + Math.max(0, schedule.drive_time_pad_minutes)
}
