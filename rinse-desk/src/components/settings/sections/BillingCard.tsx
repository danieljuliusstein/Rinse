import { startDesktopBillingPortal, startDesktopCheckout } from '@/lib/billing-checkout'
...
export function BillingCard() {
  ...
  async function open(path: string) {
    ...
    try {
      ...
      if (path === '/api/billing/portal') { await startDesktopBillingPortal(); return }
  ...
  return <Card><CardHeader title="Billing & plan" /><CardBody className="space-y-4">
    <p>{founding ? `Founding · lifetime ${STARTER_PLAN.name}, granted by the owner` : paid ? org?.plan === 'early' ? 'Early · $3/month' : `${STARTER_PLAN.name} · $6/month` : 'Free · $0'}</p>
    ...
    {!founding && org?.billing_provider === 'apple' ? <p>Manage or restore your App Store subscription in the Rinse iOS app.</p> : !founding && <div className="flex flex-wrap gap-3">
      {!paid && !['active','past_due'].includes(org?.subscription_status || '') && <button disabled={!org || busy} className="rounded-xl bg-brand-600 text-white px-4 py-3" onClick={() => void open('/api/billing/checkout')}>Upgrade to {STARTER_PLAN.name} · $6/month</button>}
    ...
    <p className="text-sm">Cancellation keeps {STARTER_PLAN.name} through the paid period, then returns to Free. Existing records are preserved.</p>
